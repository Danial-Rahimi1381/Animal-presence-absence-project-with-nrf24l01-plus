...
void NRF24L01plus_Init()
{
    ...
    NRF24_Write_Register(SETUP_RETR, 0xFF); // Enable automatic retransmission with maximum delay and retries
...
void NRF24_TXMode( uint8_t const* Address, uint8_t channel)
 {
    ...
    // power up the device
    config = NRF24_Read_Register(CONFIG);
    config = (config | (1<<1)) & 0b11111110;
    NRF24_Write_Register(CONFIG, config);

    // Enable Enhanced ShockBurst
    NRF24_Write_Register(EN_AA, 0x02); // Enable Auto ACK on pipe 0
    NRF24_Write_Register(EN_RXADDR, 0x02); // Enable data pipe 0
    NRF24_Write_Register(DYNPD, 0x02); //Enable DPL  PIPE 0
    NRF24_WriteMulti_Register(RX_ADDR_P1, Address, 5); //Write the TX address
    delay_ms(500);
...
void nrf24_state(uint8_t state)
{
    ...
    uint8_t data;
	config_register = NRF24_Read_Register(CONFIG);
	...
	switch (state)
	{
		...
		NRF24_Write_Register(STATUS,data);
		break;
		case TRANSMIT:
		data = config_register & ~(1 << PRIM_RX);
		NRF24_Write_Register(CONFIG,data);
...
uint8_t NRF24_Transmit(uint8_t* data , uint8_t length)
 {
	...
	NRF24_Write_Register(FLUSH_TX,0);
	data_reg = (1 << TX_DS);
	NRF24_Write_Register(STATUS,data_reg);
  ...
  CSN_Enable;
    ...
    SPI_Transmit_nrf24(data,length);   
    PORTC.5 = 0;
  CSN_Disable;
  ...
  delay_ms(3);//10000 US
   ...
   //fifostatus = NRF24_Read_Register(FIFO_STATUS);
    ...
    if (status&(1<<5))//(fifostatus&(1<<4)) && (!(fifostatus&(1<<3))))
    {
    PORTC.5 = 1;           
     counter  = 0;
    // cmdtosend = FLUSH_TX;
    ...
    data_reg = NRF24_Read_Register(CONFIG); 
    data_reg = data_reg & 0b11111101;
    NRF24_Write_Register(CONFIG,data_reg); 
...
void writeAckPayload(uint8_t number_pipe, uint8_t *Data, uint8_t DataSize)
    {
     uint8_t cmdtosend;
    cmdtosend = W_ACK_PAYLOAD;
    SPI_Transmit(cmdtosend |(number_pipe & 0x07));
    SPI_TransmitBytes(Data,DataSize);
